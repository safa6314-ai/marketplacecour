...
import org.example.controller.CourseManagerController;
...
public class AdminDashboardController {
    ...
    @FXML public void openCoursesCours() { loadCoursesPage("cours", btnCoursesCours); }
    @FXML public void openCoursesEtudiants() { loadCoursesPage("etudiants", btnCoursesEtudiants); }
    @FXML public void openCoursesRapport() { loadCoursesPage("rapports", btnCoursesRapport); }
    ...
    private void loadCoursesPage(String mode, Button activeButton) {
        if (SessionManager.getCurrentUser() == null) {
            redirectToLogin();
            return;
        }
        ...
        try {
            URL url = getClass().getResource("/org/example/ui/CourseManager.fxml");
            if (url == null) {
                throw new IOException("FXML introuvable : /org/example/ui/CourseManager.fxml");
            }
            ...
            if (controller != null) {
                switch (mode) {
                    case "etudiants" -> controller.showStudentsPage();
                    case "rapports" -> controller.showReportsPage();
                    default -> controller.showCoursesPage();
                }
            }
            ...
            showAlert(Alert.AlertType.ERROR, "Erreur Courses", e.getMessage());
            e.printStackTrace();
        }
    }
