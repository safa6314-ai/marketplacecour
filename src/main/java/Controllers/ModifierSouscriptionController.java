...
public class ModifierSouscriptionController {
    ...
    void handleSaveEdit(ActionEvent event) {
        ...
        try {
            ...
            NotificationHelper.success("✏ Souscription modifiée", "La souscription de '" + souscription.getNomClient() + "' a été mise à jour.");
            
            closeWindow();
        } catch (Exception e) {
            NotificationHelper.error("❌ Erreur", "Erreur base de données.");
            errorLabel.setVisible(true);
            errorLabel.setManaged(true);
            e.printStackTrace();
        }
